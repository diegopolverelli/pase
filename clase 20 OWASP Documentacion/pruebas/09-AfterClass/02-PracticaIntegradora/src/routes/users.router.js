...
import { MailService, createHash } from '../utils.js';
...
router.post('/',async(req,res)=>{
    ...
    if(!first_name||!last_name||!dni||!email||!birthDate||!password) return res.status(400).send({status:"error",error:"Incomplete values"})
    ...
    let existe=await usersManager.getBy({email:email})
    if(existe) return res.status(400).send({status:'error', error:'Email existe en BD'})
